...
Edited and revised by Kirill Shchegelskiy 2023
"""
...
def asl_P56(x, fs, nbits):
    """
    Measures active speech level for audio data according to ITU-T Rec. P.56 Method B.

    Parameters
    'x': column vector of floating point speech data
    'fs': sampling rate of audio data
    'nbits': number of bits per sample, i.e. quantizing intervals

    Return
    'asl_msq': active speech level mean square energy
    ...
    'actfact': activity factor (between 0 and 1)
                This is the proportion of time that the speech is deemed active
    'c0': active speech level threshold
                This is the level in uPa above which the speech is deemed active
    """
    x0 = x
    ...
    c = 2**np.arange(-15, thres_no-15, dtype=float) # series of fixed threshold voltages
    ...
    a = np.full(thres_no, 0)  # series of activity counts for each level threshold
    ...
    for k in range(x_len):
        for j in range(thres_no):
            if q[k] >= c[j]:
                # envelope is at or above j-th threshold
                a[j] = a[j]+1
                hang[j] = 0
            elif hang[j] < I:
                # envelope is below j-th threshold but hangover has not yet expired
                a[j] = a[j]+1
                hang[j] = hang[j]+1
            else:
                # speech is inactive as it is below j-th threshold
                break
    ...
    if a[0] == 0:
        ...
        #a+=2
        AdB1=10*np.log10(np.squeeze(sq)/a[0]+eps)
    CdB1 = 20*np.log10(c[0]+eps)
    if (AdB1-CdB1 < M):
        print("Warning: first active level estimate minus threshold is below margin")
        return asl_msq, actfact, c0
    ...
    for j in range(1, thres_no):
        AdB[j] = 10*np.log10(np.squeeze(sq)/(a[j]+eps)+eps)
    ...
    for j in range(1, thres_no):
        if a[j] != 0:
            ...
            if Delta[j] <= M:
                ...
                actfact = (np.squeeze(sq)/x_len)/asl_msq # this is the proportion of the time that the speech is deemed "active"
...
def P56_fileAnalysis(path):
    """
    Analyses single file with P56VAD

    Returns Active Speech Level in dB, percentage of speech present and file duration in seconds
    """
    x, fs = librosa.load(path, sr=None)
    asl_msq, actfact, c0 = asl_P56(x, fs, 16)
    duration = librosa.get_duration(y=x, sr=fs)
    activityFactor = actfact
    asldB = 10*np.log10(asl_msq+np.finfo(float).eps)

    return asldB, activityFactor, duration
...
def main(args):
    ...
    for wavPath in wavFilePaths:
        asldB, actFactor, dur = P56_fileAnalysis(dirPath+"/"+wavPath)
        print('File name {} Active Speech Level in dB {:.3f}  Activity factor {:.3f} Speech duration in seconds {:.3f}'.format(Path(wavPath).name[:4], asldB, actFactor, dur*actFactor))
